-- =====================================================================
--  Flight Operation · import_aircraft_status_2026_09_16.sql
--  สถานภาพเครื่อง + ข้อขัดข้องประจำเครื่อง + รายการเช็ควงรอบ (16 ก.ย. 2569)
--  จากไฟล์ "จัดบิน ฝูง.603 16 ก.ย.69 แก้ไขจัดบินครั้งที่1.xlsx"
--   • ต้องรัน migration_17, 18, 19 ก่อน
--   • NMCS ในกระดาน → เก็บเป็น NMC (สีแดง) เพราะระบบใช้ FMC/PMC/NMC · ใส่ NMCS ไว้ในหมายเหตุ
--   • บันทึกประวัติสถานภาพ (aircraft_status_history) ของวันที่ 2026-09-16 ด้วย
--   • รันซ้ำได้: แทนที่ข้อขัดข้องประจำเครื่อง/รายการเช็ค ของแต่ละเครื่องทั้งชุด
...
begin
  ...
  if v_id is not null then
    update public.aircraft set status = 'FMC' where id = v_id;
    insert into public.aircraft_status_history (aircraft_id, log_date, status, note)
    values (v_id, '2026-09-16', 'FMC', null)
    ...
    insert into public.aircraft_checks (aircraft_id, seq, name, due_text) values (v_id, 1, '2 M', '21 - 22 ก.ย.69');
    insert into public.aircraft_checks (aircraft_id, seq, name, due_text) values (v_id, 2, 'A11-Cx', '21 - 22 ก.ย.69');
  ...
  if v_id is not null then
    ...
    insert into public.aircraft_status_history (aircraft_id, log_date, status, note)
    values (v_id, '2026-09-16', 'NMC', 'สถานภาพเดิม: NMCS')
    on conflict (aircraft_id, log_date) do update set status = excluded.status, note = excluded.note, updated_at = now();
    delete from public.aircraft_defects where aircraft_id = v_id;
  ...
  if v_id is not null then
    update public.aircraft set status = 'NMC' where id = v_id;
    insert into public.aircraft_status_history (aircraft_id, log_date, status, note)
    values (v_id, '2026-09-16', 'NMC', null)
    on conflict (aircraft_id, log_date) do update set status = excluded.status, note = excluded.note, updated_at = now();
    delete from public.aircraft_defects where aircraft_id = v_id;
...
select a.tail_number as "เครื่อง", a.status as "สถานภาพ",
       (select count(*) from aircraft_defects d where d.aircraft_id=a.id) as "ข้อขัดข้องประจำเครื่อง",
